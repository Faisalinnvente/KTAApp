//
//  UpdateTargetWeightViewController.swift
//  KTA
//
...
class UpdateTargetWeightViewController: UIViewController, NMMultiUnitRulerDelegate, NMMultiUnitRulerDataSource {
    ...
    var targetWeight: Double = 90.0
    
    override func viewDidLoad() {
        super.viewDidLoad()

        Analytics.logEvent("view_update_target_weight", parameters: nil)
        initViews()
    }
    ...
    @IBAction func saveTargetWeight(_ sender: Any) {
        UserDefaults.standard.set(targetWeight, forKey: "targetWeight")
        Analytics.logEvent("update_target_weight", parameters: ["target_weight": targetWeight])
